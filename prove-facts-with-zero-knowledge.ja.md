...
2. 正規化された属性を Poseidon ベースのマークルツリーで**コミット**し、`attrCommitmentRoot` を生成。
...
回路 `age-over-18` は `age_bucket` が `adult` に等しいかを検査します。実際の年齢は見ません — コミットされた正規化値とコミットメントを開くランダムネスのみを扱います。`prepare` が返す `commitments` オブジェクトには、マークルツリーに使用されるハッシュアルゴリズムを指定する `scheme` フィールド（デフォルト: `"poseidon"`）が含まれます。
