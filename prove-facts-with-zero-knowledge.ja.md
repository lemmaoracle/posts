...
「このユーザーは 18 歳以上か？」はビジネスルールです。「コミットされた属性 `age_bucket` が回路 `age-over-18` の下で `adult` に等しく、チェーン 1 上の `commitmentRoot` に対して検証済み」は証明可能な事実です。
...
2. 正規化された属性を Poseidon ベースのマークルツリーで**コミット**し、`commitmentRoot` を生成。
...
回路 `age-over-18` は `age_bucket` が `adult` に等しいかを検査します。実際の年齢は見ません — コミットされた正規化値とコミットメントを開くランダムネスのみを扱います。`prepare` が返す `commitments` オブジェクトには、マークルツリーに使用されるハッシュアルゴリズムを指定する `scheme` フィールド（デフォルト: `"poseidon"`）が含まれ、`root`、`leaves`、`randomness` が含まれます。
