...
Lemma の「ガードレールをシステムの内側に組み込む」アプローチは、この軸の上に乗ります。Bourzikas は実装手段としてアーキテクチャ設計と運用プロセスを挙げ、Lemma は同じ軸を暗号証明で実装します。検証可能性と事前性を満たす来歴証明レイヤーは、「バグが存在しても攻撃が成立しない」状態を、システムの内側からコードと数学で保証する形になります。

必要なのは監視の強化ではなく、ガードレールをシステムの内側に組み込むことです。暗号証明で「何が・誰に・どこまで許可されたか」を取引前に証跡化する。事後の証跡探索ではなく、事前の証跡生成です。検証可能性（第三者が後から検証できる）と事前性（取引成立前に証跡が生成される）の二条件を満たす層が、来歴証明レイヤーの実体です。
