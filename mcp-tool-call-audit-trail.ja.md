...
アクセス制御と証跡の真正性は、別々に手当てする必要がある別の問題です。前者は「起こさせない」、後者は「起きたことを、あとから誰でも同じ結論に到達できる形で残す」。この記事が扱うのは後者です。実行前の確認と、実行後に残る記録も同じように別ものです。公共警報を題材にしたこの切り分けは[その情報は、誰から届いたのか](https://lemma.frame00.com/ja/blog/j-alert-spoofing-origin-verification-gap/)で扱っています。
