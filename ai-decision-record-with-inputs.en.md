...
The way to close both gaps is to have three things in place for every single AI decision:

- Lock the content of the decision into a single unit
- Embed the input data that decision referenced into the record of the decision itself
- Register that locked value somewhere its creator cannot rewrite
...
Start with the question of which one to lock first.
...
But a hash kept in the same place as the decision record can simply be recomputed with it: whoever created both can redo both. The hash is registered somewhere its creator cannot rewrite — in Lemma's case, a registry. Only then can the receiving side compare the registered value against the record in hand, without asking the issuer for anything.

Hashes and registries are also language your IT department will already recognize, which makes them a workable place to start the conversation.
...
## From a seismic intensity 5 Upper bulletin to the completed inspection record
...
**1. Ingest the official bulletin.** You keep the JMA epicenter and seismic-intensity report exactly as it stood when it was retrieved, and register the hash computed from it together with a pointer to the publisher — one extra step inside the ingest job.

**2. The AI issues a decision.** "A seismic intensity of 5 Upper was reported, so emergency inspection of the affected area is triggered" is recorded, pointing at the registration from step 1.
...
All four are covered by the same single trail. The scramble to gather evidence for every report and audit goes away, and **the scope you had been holding back — "we can't explain it, so we can't delegate it" — can widen, now with something to base that call on.**
...
- Where a referenced input was registered, that the registration exists

What this means is that **you no longer need anyone to take the report on faith**. In an audit, in an incident report, in sorting things out with the party that outsourced the work, the recipient can judge on the spot whether the records line up. No submitting originals on the side, no waiting on a phone call to confirm. For an input carried as a hash alone — a sensor reading whose copy stays in your own records — attaching that copy lets the receiving side check it the same way.

What the record covers is the content and the path of the decision, not an evaluation of whether the decision was right. That belongs to the people on site. The same holds for the inputs. For a registered input, the record's job extends as far as letting you follow the reference to the publishing source; for one whose copy you keep, as far as confirming that the copy and the hash agree.
