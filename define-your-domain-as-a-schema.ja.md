...
スキーマがなければ、あるドキュメントの「温度」フィールドは摂氏かもしれないし、別のドキュメントでは華氏かもしれません。「年齢」フィールドはあるレコードでは整数、別のレコードでは生年月日かもしれません。ZK 証明は曖昧なデータでは機能しません — 比較が明確に定義された正規化済みの型付き属性空間が必要です。
...
スキーマ、ジェネレーター、回路は同じパターンで登録されます：メタデータはサーバー側レジストリに、アーティファクト（wasm、zkey）は IPFS/HTTPS に保存され、ID は他の API から参照されます。新しいドメインの追加はスキーマの登録、normalize 関数の作成、そして任意で回路とジェネレーターの作成を意味します。コアプロトコルの変更は不要です。
