...
- **コミットメントと ZK**
  - 正規化された属性は **Poseidon ベースのマークルツリー** でコミットされ、`attrCommitmentRoot` および ZK のウィットネス/公開入力として使う属性ごとのコミットメントを生成する。
...
- **アーキテクチャと信頼**
  - **5 層モデル**:
    1. **オンチェーン来歴**: `LemmaRegistry` が `docHash`、`attrCommitmentRoot`、スキーマ、発行者/主体 ID、失効データを保存。ZK 検証器コントラクトは `ProofVerified` イベントを発行。
...
// prep.normalized, prep.commitments (scheme, attrCommitmentRoot, perAttributeCommitments, randomness を含む)
```
