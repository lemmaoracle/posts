...
- **ゼロ知識証明**: 任意の JSON ドキュメント（契約、ログ、申請など）は暗号化されたまま、Lemma は特定の条件を満たすかどうかのみを証明します。ブロックチェーンと組み合わせることで、機密性、条件充足の主張、公開かつ改ざん検知可能な来歴を提供します。
...
- **コミットメントと ZK**
  - 正規化された属性は **Poseidon ベースのマークルツリー** でコミットされ、`commitmentRoot` および ZK のウィットネス/公開入力として使う属性ごとのコミットメントを生成する。
  - `commitments` オブジェクトには `scheme` フィールド（デフォルト: `"poseidon"`）が含まれ、コミットメントに使用するハッシュアルゴリズムを指定します。これにより将来の Poseidon2 などの他の ZK フレンドリーなハッシュへの移行が可能です。
...
- **オンチェーン来歴とフック**
  - `LemmaRegistry` コントラクトは `docHash`、`commitmentRoot`、スキーマ、発行者/主体 ID、失効データ、および検証器コントラクトへのリンクを保存する。
  - ドキュメント登録時に、レジストリ呼び出しと同じ来歴（公開入力）を受け取るオプションのスマートコントラクトフックを実行可能。
- **アーキテクチャと信頼**
  - **5 層モデル**:
    1. **オンチェーン来歴**: `LemmaRegistry` が `docHash`、`commitmentRoot`、スキーマ、発行者/主体 ID、失効データを保存。ZK 検証器コントラクトは `ProofVerified` イベントを発行。
    2. **オフチェーンストレージ**: 暗号化ドキュメントは IPFS/Ceramic に保存。CID と `docHash` のマッピングはクライアント側で管理。
    3. **暗号化インデックス / ZK**: 構造化暗号化インデックスと ZK 証明生成/検証コンポーネント（Circom/Halo2）。
    4. **オラクルコア / レジストリ / クエリゲートウェイ**: ドキュメント登録、証明送信、検証済み属性クエリ、スキーマ/回路/ジェネレータ登録のための API エンドポイント。
    5. **クライアント / AI / RAG / dApps**: TypeScript/Go SDK 経由で Lemma API を利用。
...
- **課題:** AI 学習には検証可能な来歴と公正なクリエイター報酬が欠けている。
...
一行説明: `encrypt` は `docHash`/`cid`/`ciphertext`/`algorithm` を、`prepare` は正規化された属性とコミットメント（`scheme` を含む）を返します。
