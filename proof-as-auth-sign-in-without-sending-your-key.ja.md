...
**サーバーが保存するもの。** Bearerトークン方式ではサーバーが秘密そのもの(あるいは直接比較で検証できる形式)を保存します。リフレッシュトークン方式では長命なトークンをサーバー側で保持します。どちらもDB漏洩で認証情報が露出します。Sealプルーフ認証でサーバーが保存するのは鍵のハッシュだけです。しかもそのハッシュは認証時にネットワークを渡りません。サインインリクエストを傍受した攻撃者が見るのはnullifierだけ――鍵について何も明かさず、再利用もできないセッション固有の値です。
