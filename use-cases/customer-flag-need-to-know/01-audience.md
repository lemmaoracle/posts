---
title: "For teams running high-risk customer flags"
---


You run internal "handle with care" flags on customers with past trouble, but showing the reason to the front line invites leakage and friction, while hiding it leaves you unable to later show the flag was legitimate — this is for teams caught in that bind.

- Hospitality, retail & services, and membership businesses running customer-handling flags
- Finance teams that must share high-risk-customer or transaction-restriction flags with the front line
- Roles accountable for explaining the basis of a flag to complaints, subject-access requests, or audits
