...
もう一方の **エージェント側（払い手の側）の信頼レイヤー** — 「払ったエージェントは誰で、どの権限で、どのポリシー下で支払ったか」 — にも、独立したリファレンス実装が用意されました（[github.com/lemmaoracle/trust402](https://github.com/lemmaoracle/trust402)）。本記事が扱う資源側では払い手はまだウォレットアドレスとして同定されるだけですが、`did:key` への紐付けやロール・spendLimit の証明（ZK 述語）はエージェント側レイヤーが担います。
...
エージェント側の信頼レイヤー — DID バインディング、ロール・スコープ・spendLimit の検証 — にも、独立したリファレンス実装が用意されました（[lemmaoracle/trust402](https://github.com/lemmaoracle/trust402)）。次回のブログでは、資源側と組み合わさったときの全体像をお見せする予定です。
