...
relatedLinks:
  ...
  - label: "trust402（エージェント側リファレンス実装）"
...
もう一方の **エージェント側（払い手の側）の信頼レイヤー** — 「払ったエージェントは誰で、どの権限で、どのポリシー下で支払ったか」 — も整いました。Trust402 のエージェント側リファレンス実装は [github.com/lemmaoracle/trust402](https://github.com/lemmaoracle/trust402) にあります。本記事が扱う資源側では払い手はまだウォレットアドレスとして同定されるだけですが、`did:key` への紐付けやロール・spendLimit の証明（ZK 述語）はエージェント側レイヤーが担います。
...
エージェント側の信頼レイヤーは、払い手のエージェントを「匿名のウォレット」から「検証可能なプリンシパル」に格上げします。

具体的には、`did:key` によるプリンシパル同定、ロールとスコープによる権限表現、spendLimit による決済上限の明示、そしてクレデンシャルの発行・失効といったライフサイクルを、ZK 述語として表現し検証可能にします。

エージェント側レイヤーが加わると、`PAYMENT-RESPONSE` には「この支払いは `did:key:...` のエージェントが、`org:acme` の `payments:read` スコープ下で、spendLimit 以内・有効期間内・未失効の状態で実行した」という証明が同梱されます。資源側の信頼レイヤーと組み合わさることで、ホワイトペーパーで予告した「自律決済の信頼レイヤー」が完成します。
...
Trust402 のエージェント側信頼レイヤー — DID バインディング、ロール・スコープ・spendLimit の検証 — のリファレンス実装が用意されました（[lemmaoracle/trust402](https://github.com/lemmaoracle/trust402)）。次回のブログでは、資源側と組み合わさったときの全体像をお見せする予定です。
